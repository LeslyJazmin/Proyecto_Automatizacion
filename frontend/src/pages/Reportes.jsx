...
export default function Reportes() {
  ...
  const [vencidos, setVencidos] = useState([]); // ✅ NUEVO
  ...
  useEffect(() => {
    const cargarAlertas = async () => {
      try {
        ...
        // ❌ PRODUCTOS YA VENCIDOS - NUEVO
        const comestiblesVencidos = comestibles.filter((p) => {
          if (!p.fecha_vencimiento) return false;
          const fechaVenc = new Date(p.fecha_vencimiento);
          return fechaVenc < hoy;
        });

        const listaVencidos = comestiblesVencidos.map((p) => ({
          tipo: "Comestible",
          nombre: p.nombre,
          fecha_vencimiento: p.fecha_vencimiento,
          stock: p.stock_actual ?? 0,
  ...
  <div className="min-h-screen bg-gray-100">
    ...
    <div className="p-8 md:ml-72 ml-0 mt-20 md:mt-0">
      ...
      {vencidos.length > 0 && (
        <div className="p-4 mb-6 bg-red-200 border-l-4 border-red-700 rounded-lg shadow">
          <h2 className="font-bold text-red-800 text-lg">❌ Productos Vencidos</h2>
          <ul className="text-red-800 text-sm ml-4 list-disc">
            {vencidos.map((p, i) => {
              const fechaFormateada = new Date(p.fecha_vencimiento).toLocaleDateString("es-PE");
              return (
                <li key={i}>
                  <strong>{p.tipo}:</strong> {p.nombre} — Vencido el: <b>{fechaFormateada}</b>
                </li>
              );
            })}
          </ul>
